...
module.exports = {
    ...
    common:{
        ...
        /**
         * AD.test.common.accessedOnlyViaMethods()
         *
         * verifies a given resource route can only be accessed using the 
         * specified methods: REST, shortcuts, actions
         *
         * @codestart
         ...
         * @codeend
         *
         * If a method is not specified in the opt parameter, then that check is skipped.
         *
         * @param {json} opt    testing options.  Avaliable parameters:
         *                      .url {string}  [REQUIRED] the resource url to check
         *                      .request {obj} [OPTIONAL] an authenticated request object
         *                                      AD.test.request()
         *                      .rest {bool}    [OPTIONAL] test the REST interface. true: accessible. 
         *                      .shortcuts {bool}  "
         *                      .actions {bool}    "
         * @param {object} assert the chai.assert object for verifying your data.
         * @param {fn} ok       the callback for ending this test.
         */
        accessedOnlyViaMethods: function(opt, assert, ok) {

            if (!opt || !opt.url) {
                assert.ok(false, ' called AD.test.common.accessedOnlyWithPermission() with no .url value');
                cb();
                return;
            }


            if (typeof opt.rest == 'undefined') opt.rest = true;
            if (typeof opt.shortcuts == 'undefined') opt.shortcuts = false;
            if (typeof opt.actions == 'undefined') opt.actions = false;


            var request = opt.request || null;


            function testURL(verb, url, code, error, cb) {
                request[verb](url)
                    .set('Accept', 'application/json')
                    .expect(code)                        // should return a not found 
            ...
            function verifyAccess(verb, url, error, cb) {
                testURL(verb, url, 200, error, cb);
            }

            function verifyNoAccess(verb, url, error, cb) {
                testURL(verb, url, 404, error, cb);
            }
            ...
            async.series([
                ...
                function(next){
                    ...
                    // if they don't provide a .rest option, then skip:
                    if (typeof opt.rest == 'undefined') {
                        next();
                        return;
                    }

                    if (opt.rest) {
                        verifyAccess('get', opt.url, 'REST routes accessible', next);
                    } else {
                        verifyNoAccess('get', opt.url, 'REST routes not accessible',next);
                    }
                },

                // verify shortcut access
                function(next){
                    
                    // if they don't provide a .shortcuts option, then skip:
                    if (typeof opt.shortcuts == 'undefined') {
                        next();
                        return;
                    }

                    if (opt.shortcuts) {
                        verifyAccess('get', opt.url+'/find', 'SHORTCUT routes accessible', next);
                    } else {
                        verifyNoAccess('get', opt.url+'/find', 'SHORTCUT routes not accessible', next);
                    }
                },
                ...
                function(next){
                    ...
                    // if they don't provide a .actions option, then skip:
                    ...
                    if (opt.actions) {
                        verifyAccess('get', opt.url+'/_unitTestAccessActions', 'ACTION routes accessible', next);
                    } else {
                        verifyNoAccess('get', opt.url+'/_unitTestAccessActions', 'ACTION routes not accessible', next);
                    }
                }
         ...
         *                                 [default: 'get']
         *                  .request {obj} [optional] An authenticated request object to use
        ...
        accessedOnlyWithPermission: function(opt, assert, cb) {
            ...
            async.series([
                ...
                function(next) {
                    // if we were provided an authenticated request object, use that.
                    if (request) {
                        next();
                        return;
                    }
