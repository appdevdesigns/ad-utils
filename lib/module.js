...
//AD.log(listFiles);
                ...
                function(next) {
                    ...
                    linkAssets({sailsDir:sailsDir, moduleDir:moduleDir, files:filesToIgnore});
                    next();
                },
...
//AD.log('ignoreList:', options.ignore);
                    ...
                    patchLocal({moduleDir:moduleDir, ignore:ignoreList }, next);
...
var expectedConfigFiles = 'config.js, routes.js, policies.js, adapters.js';
                       ...
                       // recursive linkDirectory()
                        ...
                        if (stat.isSymbolicLink()) {
                            ...
                            if ((linkInfo.indexOf(moduleDir) == 0)
                                ...
                                AD.log('<white><bold>exists:</bold> '+shortPath(options.sailsDir, pathSails)+' (ours) </white>');
...
var patchLocal = function(options, done) {
    ...
    //// Apply the Adapters to the config/local.js adapters file
    var pathConfig = path.join(sailsDir, 'config', 'local.js');
    var pathAdapters = path.join(moduleDir, 'config', 'adapters.js');

    var configObj = require(pathConfig);
    var adaptersObj = require(pathAdapters);

    if (typeof configObj.adapters == 'undefined') {
        AD.log('<yellow><bold>WARN:</bold> config/local.js did not have an adapters set!</yellow>');
    }

    for (var a in adaptersObj) {
        if (typeof configObj.adapters[a] == 'undefined') {
            configObj.adapters[a] = adaptersObj[a];

            AD.log('<green><bold>patch:</bold> added adapter config ['+a+']</green>');
        } else {
            AD.log('<yellow><bold>SKIP:</bold> adapter config ['+a+'] already exists ... skipping</yellow>');
        }
    }

//AD.log('pathConfig:'+pathConfig);
//AD.log('pathAdapters:'+pathAdapters);
//
//
//AD.log(adaptersObj);
...
//AD.log('file:'+file);
    ...
    if (portData) {
        if (portData[1]) {
            ...
            configData = configData.replace(/^\s*"port":.+$/m, '    "port": '+portData[1]);
        }
    }
    ...
    if (environmentData) {
        if (environmentData[1]) {
            ...
            configData = configData.replace(/^\s*"environment":.+$/m, '    "environment": '+environmentData[1]);
        }
    }
    ...
    files.forEach(function(file) {
        ...
        if ('.gitkeep '.indexOf(file) == -1) {
            ...
            if (!fs.existsSync(sailsPath)) {
                ...
                AD.log('<green><bold>Linked:</bold>'+shortPath(options.sailsBase, sailsPath)+'</green>');
