...
module.exports = {
         ...
         * @function bootstrap
         *
         * performs a common set of checks during our bootstrap process for our
         * modules.
         *
         * @param {string} dirBootstrap The full directory to our module's 
         *                  config/bootstrap.js  file.  From this directory we
         *                  will check for the additional setup files that need
         *                  to be processed.
         * @param {fn} cb  The callback routine that is called once all our 
         *                  standard setup routines are complete.
         */
        bootstrap: function(dirBootstrap, cb) {

            async.series([

                // make sure our permissions are setup:
                function(next) {
                    AD.module.permissions(path.join(dirBootstrap, '..', 'setup', 'permissions'), next);
                },

                // make sure our OpsTool Defintions are setup:
                function(next) {
                    OPSPortal.NavBar.ToolDefinition.setupPath(path.join(dirBootstrap, '..', 'setup', 'opstools', 'opstools.js'), next);
                }
...
// AD.log('   ... Directory: '+filePath);
        ...
        /**
         * @function AD.module.permissions
         *
         * perform any permission setup steps:
         *  1) install action definitions
         *  2) install route permission
         *
         * @param {string} dir  the directory of the module running this
         *                      dir can also be the directory of the setup/permissions 
         * @param {fn} done  the callback for when this operation is done
         */
        permissions:function(dir, done) {

            // var apath = path.join(dir, 'actions.js');
            // if (fs.accessSync(apath, fs.R_OK))
            // Read in the action definitions and pull out our info
            // var fpath = path.join(dir, '..', '..', 'setup', 'permissions', 'actions.js');
            var actionPath = null;
            var routePath = null;
            var actions = null;  // require(fpath);
            var language_code = null; // actions.language_code || Multilingual.languages.default();
            var actionList = [];  // actions.actions;   // an array of action definitions


            //
            // processOne
            // a recursive fn() to process a single action definition
            // if actionList is empty, then we are done.
            //
            var processOne = function(cb) {
                ...
                if (actionList.length) {
                    ...
                    .then(function(list) {
                        if (list.length == 0) {
                            def.language_code = def.language_code || language_code;
                            sails.log.info('creating: permission action:'+def.action_key);
            ...
            //
            // allDone
            // The callback that determines if all our parallel tasks
            // are complete.  If they are, then we done();
            //
            var numPar  = 5;    // how many parallel 'threads'
            var numDone = 0;    // how many 'threads' have completed
            var hasDone = false;    // have we already reported an err?
            ...
            async.series([
                ...
                function(next) {
                    ...
                    // is dir a path/to/setup/permissions  ?
                    var apath = path.join(dir, 'actions.js');
                    // fs.access(apath, fs.R_OK, function(err){
                    fs.exists( apath, function(exists) {

                        if (!exists) {
                            // nope, try a dir from a calling .__init() routine
                            var fpath = path.join(dir, '..', '..', 'setup', 'permissions', 'actions.js');
                            // fs.access(fpath, fs.R_OK, function(err) {
                            fs.exists(fpath, function(exists) { 

                                if (!exists) {
                                    // i can't figure this out!
                                    var err = new Error('Cant determine permissions from directory:'+dir);
                                    next(err);

                                } else {

                                    actionPath = fpath;
                                    routePath = path.join(dir, '..', '..', 'setup', 'permissions', 'routes.js');
                                    next();
                                }
                            ...
                            actionPath = apath;
                            routePath = path.join(dir,'routes.js');
                            next();
                        }
                    })
                ...
                // step 2: open the right file and setup our values
                function(next) {

                    actions = require(actionPath);
                    language_code = actions.language_code || Multilingual.languages.default();
                    actionList = actions.actions;   // an array of action definitions
                ...
                // step 3: start off our recursive functions
                function(next) {

                    if (actionList.length > 0) {

                        // start this many operations in parallel:
                        if (numPar > actionList.length) {
                            numPar = actionList.length;
                        }
                ...
                // step 4:  also register any route permissions:
                function(next) {
                    ...
                    fs.exists( routePath, function(exists) {
                        ...
                        if (exists) {
                            ...
                            var routeDefinitions = require(routePath);

                            for(var d in routeDefinitions) {
                                Permissions.route.registerDefinition( d, routeDefinitions[d] );
                            }
                ...
                //// NOTE:  the allDone() fn() above determines our actual callback.
        ...
        setup:function( options ) {
            ...
            async.series([
                ...
                // 2nd: prepare file links by 1st removing ALL links to files in our module
                function(next) {
                ...
                // 3rd: create any symbolic links back to our directories
                function(next) {
                    AD.log();
                    ...
                    for (var k in allLinks) {
                        ...
                        // the sailsLink includes a directory at the end.  to 
                        // make this a proper relative link we need to remove
                        // the final directory
                        var linkParts = sailsLink.split(path.sep);
                        linkParts.pop();
                        var relBase = linkParts.join(path.sep);
                        ...
                        if (!fs.existsSync(sailsLink)) {
                            var relativePath = path.relative(relBase, currDir);

// fs.symlink(relativePath, '/some-dir/foo', callback);
                            // fs.symlinkSync(currDir, sailsLink, 'dir');
                            fs.symlinkSync(relativePath, sailsLink, 'dir');
                ...
                function(next) {
                    ...
                    var ignoreList = options.ignore.assets || {};

                    linkAssets({sailsDir:sailsDir, moduleDir:moduleDir, files:filesToIgnore, isRemove:false, ignoreList:ignoreList });
                    next();
                },
                ...
                // do this after all the filesystem work is done:

                // // now check to make sure all the action keys are installed:
                // function(next) {

                //     installActionKeys(next);

                // }


                // 9th : now make sure all labels are imported properly.
                function(next) {

                    AD.log();
                    AD.log('<green>import:</green> labels.');
                    AD.log();

                    // make sure we include a link to the parent sails .adn file:
                    var pathModuleADN = path.join(moduleDir,'.adn');
                    var pathSailsADN  = path.join(sailsDir,'.adn');
                    try {
                        var stat = fs.lstatSync(pathModuleADN);
                        if (!stat.isFile() && !stat.isSymbolicLink()) {
                            // make a symbolic link from our module to our sails .adn:
                            fs.symlinkSync(pathSailsADN, pathModuleADN)
                    ...
                    // now run the appdev labelImport  command:
                    var pathCWD = process.cwd();
                    process.chdir(moduleDir);

                    
                    AD.spawn.command({
                        command:'appdev',
                        options:[ 'labelImportSetup'],
                        // shouldEcho:false,
shouldEcho:true
                        // responses:responses
                    })
                ...
                if (err) {
                    AD.log();
                    AD.log.error("setup did not complete properly.");
                    AD.log(err);
                    process.exit(1);
                } else {
                    ...
                    AD.log();
                    AD.log('<yellow>setup finished</yellow>');


                    // TODO: 
                    // I think Sails is keeping this alive, so kill our process.
                    // or figure out how to tell Sails.lower() ...
                    process.exit();
                }
            });
...
var filesInThisModule = function(options) {
    ...
    if (!fs.existsSync(options.dir)) {
        AD.log('creating directory: ' + options.dir);
        fs.mkdirSync(options.dir);
        
        // existsSync sometimes returns before the directory is fully created.
        // So keep waiting if needed.
        while (!fs.existsSync(options.dir)) {
            process.nextTick();
            setImmediate();
        }
...
/**
 * @function installActionKeys
 *
 * makes sure any action keys defined in /setup/permission_actions.js are
 * installed.
 *
 * @return {bool}  is the current directory a buildable project.
 */
// var installActionKeys = function(done) {

//     // by this time we are running from the Module's root Directory
//     // so we need to look in our 'setup' folder.
//     AD.log();
//     AD.log('Verifying Action Keys:');

//     var actionsPath = path.join(process.cwd(), 'setup', 'permission_actions.js');
// // AD.log('<yellow><bold>actionsPath:</bold></yellow>'+actionsPath);

//     // if file exists
//     if ( fs.existsSync(actionsPath)) {
//         // load data
//         var actionsInfo = require(actionsPath);

//         var modelAction = null; 

//         // load Sails and get the PermissionAction model
//         var Sails = null;


//         async.series([

//             // load Sails so we can get our our Actions Model
//             function(next) {
// AD.log('... loading Sails');
...
//             // get our Actions Model from Sails
//             function(next) {
// AD.log('... checking PermissionAction model');

//                 if ((Sails.models) 
//                     && (Sails.models.permissionaction)) {

//                     modelAction = Sails.models.permissionaction;
//                     next();
                    
//                 } else {

//                     AD.log.error("error: could not find sails.models.<bold>permissionactions</bold> ");

//                     next( new Error('Sails model did not have permissionaction!'));
//                 }
//             },


//             // now step through the given Action Definitions
//             function(next) {
...
//                     if ((err)
//                         || (numDone >= allKeys.length)) {
//                         next(err);
//                     }
//                 }
// AD.log('... actionsInfo:', actionsInfo);

//                 var numDone = 0;
//                 var allKeys = _.keys(actionsInfo);
//                 var languageCode = actionsInfo.code || Sails.config.appdev['lang.default']
//                 allKeys.forEach(function(key){
// AD.log('... key:'+ key);

//                     if (key != 'code') {
                        
//                         var desc = actionsInfo[key];

//                         modelAction.find({ action_key:key }).populate('translations')
//                         .then(function(list){

//                             if (list.length > 0) {
//                                 AD.log('<white><bold>exists:</bold></white> '+key);
//                                 allDone();
//                             } else {
...
//                                 })
//                                 .then(function(data){
//                                     AD.log('<green><bold>added:</bold></green> '+key);
//                                     allDone();
//                                 });
...
//                     } else {
//                         // make sure we include this in our allDone counts.
//                         allDone(numDone);
//                     }
//                 })

// // AD.log('.... ready to process actions!');
// // next();
//             }
...
//             // make sure our Sails closes down!
// // AD.log('... done()');     
//             Sails.lower(function(){

// // AD.log('... sails.lowered() ');
//                 done(err);
//             });
//         })
        

//         // for each defined entry
//             // if not already defined,  create it.

//     } else {

//         AD.log('<yellow>warn:</yellow> no action permissions defined.');
//         done();
//     }
// };
...
var linkAssets = function( options ) {
    ...
    var ignoreAssets = options.ignoreList || {};
    // ignore Assets was given without the assets/ path, so convert them to have that:

    var ignoreAssetsFull = [];
    for (var k in ignoreAssets) {
        ignoreAssetsFull.push( path.join('assets', k) );
    }

// AD.log('... ignoreAssetsFull:', ignoreAssetsFull);
    ...
    var linkDirectory = function ( pathDir ) {
        ...
        files.forEach(function(file) {
            ...
            // if not a common file reference I should ignore:
            if ('.gitkeep .. .DS_Store'.indexOf(file) == -1) {

//AD.log('linkDirectory: file:'+file);

                // if not a specific asset reference I was told to ignore
                var assetPath = path.join(pathDir, file);
                if (ignoreAssetsFull.indexOf(assetPath) == -1) {
                    ...
                    if (!fs.existsSync(pathSails)) {
                        ...
                        // verify it isn't a symLink already:
                        // I've seen fs.existsSync() report false for a symLink
                    ...
                    AD.log('<yellow><bold>ignored:</bold>  told to ignore file: '+ assetPath + '</yellow>');
                }
...
// AD.log('... buildable projects:', projects);
    ...
    var mixin = function( newValues ) {
        ...
        for (var s in newValues ) {
            ...
            // s.bundle
            else if (_.isArray(newValues[s])) {
                if (typeof StealConfig[s] == 'undefined') {
                    StealConfig[s] = newValues[s];
                    log('green', 'added:', s, '', "[]", newValues[s]);
                }
                else {
                    StealConfig[s] = _.union(StealConfig[s], newValues[s]);
                    log('white', 'merged:', s, s, '[ ' + newValues[s] + ' ]', '<yellow>merge</yellow>');
                }
            }
            else {
                if (typeof StealConfig[s] == 'undefined') {
...
var syncDirLinks = function(options) {
    ...
    // if it doesn't exist, then we are done.
    if (!fs.existsSync(options.moduleDir)) {
        return;
    }
    ...
    files.forEach(function(file) {
        ...
        // if not a common file to ignore
        if ('.gitkeep .DS_Store'.indexOf(file) == -1) {
